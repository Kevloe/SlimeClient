...
public class LayerArrowCustom implements LayerRenderer<EntityLivingBase> {
  ...
  public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale) {
    ...
    if (i > 0) {
      EntityArrow entityArrow = new EntityArrow(entitylivingbaseIn.worldObj, entitylivingbaseIn.posX, entitylivingbaseIn.posY, entitylivingbaseIn.posZ);
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (!modelrenderer.isHidden && modelrenderer.cubeList.size() != 0) {
          ...
          ((Entity)entityArrow).prevRotationYaw = ((Entity)entityArrow).rotationYaw = (float)(Math.atan2(f, f2) * 180.0D / Math.PI);
          ((Entity)entityArrow).prevRotationPitch = ((Entity)entityArrow).rotationPitch = (float)(Math.atan2(f1, f6) * 180.0D / Math.PI);
          double d0 = 0.0D;
          double d1 = 0.0D;
          double d2 = 0.0D;
          this.field_177168_a.getRenderManager().renderEntityWithPosYaw((Entity)entityArrow, d0, d1, d2, 0.0F, partialTicks);
